...
/** Magic method which allows read-only access to protected or private class properties
  ...
  * @return mixed */
        ...
        if ($config['disabled'] || !$config['access']['files']['upload']) {
            ...
            if (!strlen($message)) {
                ...
                if (!@move_uploaded_file($file['tmp_name'], $target) &&
                    ...
                    $livePath = $this->uploadImageOnServer($target); /* By Tanmay Vats */
                    $this->makeThumb($target);
                    $url = $this->typeURL; /* ie. http://studiobooth.local/admin/uploads/image */
                    if (isset($udir)) $url .= "/$udir";

                    $url .= "/" . basename($target); /* ie. filename.extension (abc.jpg)*/
                    $url = $livePath; /* Overwrite local url with live url so that editor sets image path to local.*/
    ...
    protected function uploadImageOnServer($file)
    {
        $ch = curl_init('http://app.thestudiobooth.com/app/share/wrapper/');    
        curl_setopt($ch, CURLOPT_POST, 1);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($ch, CURLOPT_POSTFIELDS, array(
            'editor_image' => (($file) ? "@$file" : null),
            'isEditorImage' => "1"
        ));
        ...
        curl_close($ch);
        if ($http_code == 200 && trim($result) != "") {
            $livePath = $result;
        } else {
            $livePath = $file;
        }
        return $livePath;
        //return " file = ".$file." -- result = ".$result." -- http_code = ".$http_code;
    }
